...
function FollowButton({ currentUserId, targetUserId }) {
  const { isFollowing, loading, toggleFollow } = useIsFollowing(currentUserId, targetUserId);
  if (!currentUserId || currentUserId === targetUserId || loading) return null;

  return (
    <button
      onClick={toggleFollow}
      className={`rounded-full px-5 py-2 text-sm font-semibold active:scale-95 ${
        isFollowing ? "bg-[var(--color-cream)] text-[var(--color-ink-soft)]" : "bg-[var(--color-olive)] text-white"
      }`}
    >
  ...
  return (
    <PageFade className="flex flex-1 flex-col gap-6 px-5 py-5">
      ...
      <section className="flex flex-col items-center gap-3 rounded-2xl bg-[var(--color-surface)] p-6 text-center shadow-sm">
        ...
        <FollowButton currentUserId={user?.id} targetUserId={id} />
      </section>
