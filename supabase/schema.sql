...
-- ============================================================
-- chat_messages: a single global, publicly-readable community chat room
-- (not per-post comments, not 1:1 DMs — one shared room off the Topluluk
-- page, matching the "chatting box where people can chat with each other"
-- requirement). Realtime is enabled via the publication line below so
-- useCommunityChat can subscribe to inserts instead of polling.
-- ============================================================
...
drop policy if exists "users can send chat messages as themselves" on public.chat_messages;
create policy "users can send chat messages as themselves"
...
-- Adds chat_messages to the publication Supabase's client-side Realtime
